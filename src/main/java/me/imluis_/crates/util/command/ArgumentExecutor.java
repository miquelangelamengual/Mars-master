package me.imluis_.crates.util.command;
...
public abstract class ArgumentExecutor implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length < 1) {
        	ChatUtil.sendMessage(sender, "&7" + BukkitUtil.STRAIGHT_LINE_DEFAULT);
        	ChatUtil.sendMessage(sender, "&4&l" + WordUtils.capitalizeFully(this.label) + " Help");
            for (CommandArgument argument : arguments) {
                String permission = argument.getPermission();
                if (permission == null || sender.hasPermission(permission)) {
                	ChatUtil.sendMessage(sender, "&c" + argument.getUsage(label) + "&7, &r" + argument.getDescription());
                }
            }
            ChatUtil.sendMessage(sender, "&7" + BukkitUtil.STRAIGHT_LINE_DEFAULT);
            return true;
